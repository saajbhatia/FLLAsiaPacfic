...
def car_windmill(hub, robot, flipper, flipperInit):
    ...
    abs_turning(hub, robot, -46, 30, 0)

    #Dump + Grab
    highspeed_pid(hub, robot, 65, 80, -46)
    abs_flip_turn(flipper, 90, 30, flipperInit)
    highspeed_pid(hub, robot, 14.5, 80, -46)

    #Do Car
    abs_turning(hub, robot, -30, 30, 0)
    abs_flip_turn(flipper, 0, 70, flipperInit)
    time.sleep(0.1)
    abs_flip_turn(flipper, 90, 30, flipperInit)
    abs_turning(hub, robot, -50, 30)

    #Back from car
    highspeed_pid(hub, robot, 28, -50, -48)
    abs_flip_turn(flipper, 90, 30, flipperInit)

    #Turn for windmill
    fast_turning(hub, robot, 60, 15)
    wait_for_seconds(0.1)
